...
/**
 * Unit tests for [PreOrderTraversal].
 */
internal class PreOrderTraversalTest {
    private val tree =
        binaryTree("A") {
            left("B") {
                left("D")
    ...
    private val traversal = PreOrderTraversal(tree)
    ...
    fun `Traversal with destination gives expected nodes in-order`() {
        ...
        val expected = listOf("A", "B", "D", "C", "E", "G")
        val actual =
            traversal.path(
                start,
                destination,
    ...
    fun `Traversal with no destination gives all nodes in-order`() {
        val expected = listOf("A", "B", "D", "C", "E", "G", "H", "F")
